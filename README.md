...
- **出走数補正**: `race_count`が少ないほど全体平均(0.33)に引き寄せる
  - **古馬**: 信頼度 = min(1, race_count / 15)
  - **3歳馬**: 信頼度 = min(1, race_count / 5)（春時点でキャリア4〜6戦が現実的上限）
  - 例（3歳）: 4戦・p3rate 1.0 → 信頼度0.80 → 補正後 0.87
...
### `recentFinishAdj`（近走着順アジャスト）
- `horse_form_records.finish_pos`（直近最大5走）から計算
- 条件戦・重賞を問わず相対着順（1着=1.0、最下位=0.0）を加重平均し圧縮（0.35〜0.65）
- 軸・ヒモスコアに `(score - 0.50) × 0.30` として加算（範囲: 約 -0.045 〜 +0.045）
- データなし: 0.0（ニュートラル）
- 3歳馬の「最近調子いい/悪い」を条件戦からも拾うための補正項
